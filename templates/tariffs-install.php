...
use models\tariffsinstall\TariffsTable;
use models\User;
...
require '_new-codebase/front/templates/main/parts/tariffs-install/tariffs-table.php';
...
if (!empty(App::$URLParams['ajax'])) {
    ...
    switch (App::$URLParams['ajax']) {
        ...
        case 'get-cats':
            $filter = TariffsTable::prepareFilter(App::$URLParams);
            $response = datatableResponse(TariffsTable::getCats($filter), $filter, TariffsTable::getFilterCnt($filter), TariffsTable::getTotalCnt());
            break;

        case 'update-cost':
            if (!empty($_SESSION['pass_protect'])) {
                $response = TariffsTable::updateCost(json_decode($_POST['data'], true));
            }
            break;
...
<head>
    <meta charset="utf-8">
    <title>Тарифы на демонтаж/монтаж - Панель управления</title>
...
<body>
    ...
    <main class="wrapper" style="max-width: 1920px">
        ...
        <h2 class="layout__mb_md">Тарифы на демонтаж/монтаж</h2>
        ...
        <section class="layout__mb_lg">
            <table id="datatable" class="display">
                <?php tableHeadHTML(TariffsTable::getCols()); ?>
            </table>
        </section>
    ...
    <script src="/_new-codebase/front/modules/tariffs-install/tariffs-table.js?v=<?= VER; ?>"></script>
    <script src="/_new-codebase/front/components/pass-protect/pass-protect.js?v=<?= VER; ?>"></script>
</body>
