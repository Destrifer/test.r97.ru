...
use models\parts\PartsTableArrivalsExcel;
use models\PartsArrivalsTable;
use models\User;
...
require '_new-codebase/front/templates/main/parts/parts-arrivals/parts-arrivals.php';
require '_new-codebase/front/templates/main/parts/parts/filter-form.php';

$filter = PartsArrivalsTable::prepareFilter(App::$URLParams);
if (!empty(App::$URLParams['ajax'])) {
  $response = [];
  switch (App::$URLParams['ajax']) {

    case 'update-arrival-name':
      $response = PartsArrivalsTable::updateArrivalName($_POST);
      break;

    case 'get-parts':
      $response = datatableResponse(PartsArrivalsTable::getParts($filter), $filter, PartsArrivalsTable::getCols($filter), PartsArrivalsTable::getFilterCnt($filter), PartsArrivalsTable::getTotalCnt(), User::getData('role'));
      break;
...
if (!empty(App::$URLParams['action'])) {
  switch (App::$URLParams['action']) {
    ...
    case 'generate-excel':
      PartsTableArrivalsExcel::generate(PartsArrivalsTable::getParts($filter), $filter);
      break;
  }
  exit;
}
...
<body>
  ...
  <main class="wrapper" style="max-width: 1920px">
    ...
    <section class="layout__mb_lg">
      <?php filterFormArrivalsHTML(
        App::$URLParams,
        PartsArrivalsTable::getCatsList(),
        PartsArrivalsTable::getCodesList(),
        PartsArrivalsTable::getModelsList(App::$URLParams),
        PartsArrivalsTable::getDepotsList(),
        PartsArrivalsTable::getPartsList(),
        Parts::$partAttrs,
        Parts::$partTypes,
        Parts::getVendors(),
    ...
    <section class="layout__mb_md">
      <?php
        secNavHTML([
          ...
          ['name' => 'Добавить приход', 'url' => '/part-arrival/']
        ]);
      ?>
    </section>
    ...
    <section class="layout__mb_lg">
      <table id="datatable" class="display">
        <?php tableHeadHTML(PartsArrivalsTable::getCols($filter)); ?>
      </table>
    </section>
  ...
  <script src="/_new-codebase/front/modules/parts-arrivals/parts-arrivals.js?v=<?= VER; ?>"></script>
