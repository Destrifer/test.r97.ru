...
require '_new-codebase/front/templates/main/parts/transport-companies/transport-companies.php';

$secNav = [
    ['name' => 'Отправить запчасти', 'url' => '/parts-ship/']
];
...
if (!empty(App::$URLParams['ajax'])) {
    ...
    switch (App::$URLParams['ajax']) {
        ...
        case 'get-companies':
            $filter = datatableFilter(App::$URLParams, Companies::getCols());
            $response = datatableResponse(Companies::getCompanies($filter), $filter, Companies::getFilterCnt($filter), Companies::getTotalCnt());
            break;
...
$secNav = [
    ['name' => 'Добавить компанию', 'url' => '/transport-company/']
  ];
...
<head>
    <meta charset="utf-8">
    <title>Транспортные компании - Панель управления</title>
...
<body>
    ...
    <main class="wrapper" style="max-width: 1920px">
        ...
        <h2 class="layout__mb_md">Транспортные компании</h2>
        ...
        <section class="layout__mb_lg">
            ...
            <table id="datatable" class="display">
                <?php tableHeadHTML(Companies::getCols()); ?>
            </table>
    ...
    <script src="/_new-codebase/front/modules/transport-companies/transport-companies.js?v=<?= VER; ?>"></script>
