...
require '_new-codebase/front/templates/main/parts/users/users-table.php';
require '_new-codebase/front/templates/main/parts/users/filter-form.php';

$secNav = [
    ['name' => 'Добавить пользователя', 'url' => '/user/']
];
...
if (!empty(App::$URLParams['ajax'])) {
    ...
    switch (App::$URLParams['ajax']) {
        ...
        case 'change-status':
            $response = UsersTable::changeStatus($_POST['new_status'], $_POST['user_id']);
            break;

        case 'get-users':
            $filter = UsersTable::prepareFilter(App::$URLParams);
            $response = datatableResponse(UsersTable::getUsers($filter), $filter, UsersTable::getFilterCnt($filter), UsersTable::getTotalCnt());
            break;
...
<head>
    ...
    <link href="/_new-codebase/front/modules/users/users-table.css?v=<?= VER; ?>" rel="stylesheet" />
    <style>
        * {
...
<body>
    ...
    <main class="wrapper" style="max-width: 1920px">
        ...
        <h2 class="layout__mb_md">Пользователи</h2>
        ...
        <section class="layout__mb_lg">
            ...
            filterFormHTML(
                App::$URLParams,
                Users::getRoles(((User::hasRole('admin')) ? [] : ['acct', 'service', 'taker', 'master'])),
                ((User::hasRole('admin')) ? Services::getServices(['is_active' => true]) : [])
            );
            ?>
        </section>
        ...
        <section class="layout__mb_lg">
            ...
            <table id="datatable" class="display">
                <?php tableHeadHTML(UsersTable::getCols()); ?>
            </table>
    ...
    <script src="/_new-codebase/front/modules/users/users-table.js?v=<?= VER; ?>"></script>
