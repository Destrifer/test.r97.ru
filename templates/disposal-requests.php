...
use models\disposals\RequestsTable;
...
require '_new-codebase/front/templates/main/parts/disposal-requests/requests-table.php';
...
if (!empty(App::$URLParams['ajax'])) {
    ...
    switch (App::$URLParams['ajax']) {
        ...
        case 'get-requests':
            $filter = RequestsTable::prepareFilter(App::$URLParams, RequestsTable::getCols());
            $response = datatableResponse(RequestsTable::getRequests($filter), $filter, RequestsTable::getFilterCnt($filter), RequestsTable::getTotalCnt());
            break;
...
if (empty(App::$URLParams['status_id'])) {
    $title = 'Активные запросы на утилизацию';
    $secNav = [
        ['name' => 'Обработанные запросы', 'url' => '/disposal-requests/?status_id=1']
    ];
} else {
    $title = 'Обработанные запросы на утилизацию';
    $secNav = [
        ['name' => 'Активные запросы', 'url' => '/disposal-requests/']
    ];
}
...
<body>
    ...
    <main class="wrapper" style="max-width: 1920px">
        ...
        <section class="layout__mb_lg">
            ...
            <table id="datatable" class="display">
                <?php tableHeadHTML(RequestsTable::getCols()); ?>
            </table>
    ...
    <script src="/_new-codebase/front/modules/disposal-requests/requests-table.js?v=<?= VER; ?>"></script>
