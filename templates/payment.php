...
while ($row = mysqli_fetch_array($sql)) {
      ...
      /*Проверяем теслер*/
      $sql3 = mysqli_query($db, 'SELECT `model_id` FROM `repairs` where `service_id` = '.$userID.' and `status_id` != 6 and `app_date` REGEXP "'.$year.'.'.$month.'" and (`status_admin` = \'Подтвержден\' or `status_admin` = \'Выдан\') and `deleted` = 0 '.$dated_sql.' group by `model_id` order by `id` DESC ;');
