...
require '_new-codebase/front/templates/main/parts/part-names-ref/part-names-ref.php';

$secNav = [
    ['name' => 'Добавить имя', 'action' => 'add-name', 'url' => ''],
    ['name' => 'Сохранить', 'action' => 'save', 'url' => '']
];
...
if (!empty(App::$URLParams['ajax'])) {
    ...
    switch (App::$URLParams['ajax']) {
        ...
        case 'add-name':
            NamesRef::addName($_POST['ru'], $_POST['en']);
            break;

        case 'save':
            $response = NamesRef::save(json_decode($_POST['data'], true));
            break;

        case 'get-names':
            $filter = NamesRef::prepareFilter(App::$URLParams);
            $response = datatableResponse(NamesRef::getNames($filter), $filter, NamesRef::getFilterCnt($filter), NamesRef::getTotalCnt());
            break;
...
<head>
    <meta charset="utf-8">
    <title>Справочник шаблонов (запчасти) - Панель управления</title>
...
<body>
    ...
    <main class="wrapper" style="max-width: 1920px">
        ...
        <h2 class="layout__mb_md">Справочник шаблонов (запчасти)</h2>
        ...
        <section class="layout__mb_lg">
            ...
            <table id="datatable" class="display">
                <?php tableHeadHTML(NamesRef::getCols()); ?>
            </table>
    ...
    <div id="add-form-window" style="display: none; min-width: 1000px">
        <form class="container gutters form" id="add-form">
            <div class="row">
                <div class="col-6"><input type="text" name="ru" required placeholder="Русский" class="form__text"></div>
                <div class="col-6"><input type="text" name="en" placeholder="Английский" class="form__text"></div>
                <div class="col-12">
                    <div class="form__field" style="display: flex; justify-content: space-between; margin-top: 32px">
                        <button class="form__btn form__btn_secondary" onclick="$.fancybox.close()">Отмена</button>
    ...
    <script src="/_new-codebase/front/modules/part-names-ref/part-names-ref.js?v=<?= VER; ?>"></script>
