  ...
  /*** PROTECTED: ***/
    ...
    protected function makeHtml()
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            $result .= ''.$litag.(($i == $page) ? '<a href="#" class="active">' : '').''.(($i == $page) ? ''.$i.'</a>' : $a).''.$sep;
        }
        ...
        if (($this->params['arr']) && ($count > 1)) {
            ...
            if ($page > 1) {
              $larr = 'Предыдущая';
                $larr = '<a class="prev" href="'.$this->makePageLink($page - 1).'">'.$larr.'</a>';
            }

            if ($page < $count) {
              $rarr = 'Следующая';
                $rarr = '<a class="next" href="'.$this->makePageLink($page + 1).'">'.$rarr.'</a>';
            }
            $result =
                ''.$larr.''.
                $result.
