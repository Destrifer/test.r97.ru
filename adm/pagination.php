  ...
  /*** PROTECTED: ***/
    ...
    protected function makeHtml()
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            $a = '<a class="paginate_button " href="'.$this->makePageLink($i).'">'.$i.'</a> ';
            $result .= ''.$litag.(($i == $page) ? '<a href="#" class="paginate_button current">' : '').''.(($i == $page) ? ''.$i.'</a>' : $a).''.$sep;
        }
        ...
        if (($this->params['arr']) && ($count > 1)) {
            ...
            if ($page > 1) {
              $larr = 'Предыдущая';
                $larr = '<a class="paginate_button previous disabled" id="table_content_previous" href="'.$this->makePageLink($page - 1).'">'.$larr.'</a>';
            }

            if ($page < $count) {
              $rarr = 'Следующая';
                $rarr = '<a class="paginate_button next" id="table_content_next" href="'.$this->makePageLink($page + 1).'">'.$rarr.'</a>';
            }
            $result =
                ''.$larr.''.
                '<span>'.$result.'</span>'.
        ...
        if ($this->params['tag']) {
            $tag = '<'.$this->params['tag'].($this->params['css'] ? ' id="'.$this->params['css'].'"' : '').'><ul>';
            $result = $tag.$result.'</ul></'.$this->params['tag'].'>';
        }
        return $result;
    }
